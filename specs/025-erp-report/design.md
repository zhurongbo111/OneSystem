...
> **演进（erp-multi-warehouse）**：`038` 为进销存报表与库存余额表追加**仓库筛选**（`warehouseId` 可空，空 = 全部仓合并），数值随之为「该仓口径」；库存余额表的按商品聚合改为 Σ 各仓（低库存按仓级阈值判定，任一仓低于阈值即计入）。口径见 `specs/038-erp-multi-warehouse/design.md` §0 / §3.4。
