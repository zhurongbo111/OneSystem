...
- 生效动作内容与既有完全一致：库存增减（按仓 / 按批次，`030` / `032` 口径）+ 库存流水 + 成本更新（`026`）+ 审计（`029`）。
...
| 提醒 | 提交时向 `approvals.approve` 用户发站内信（`035` 的 `INotificationWriter`，`NotificationType.ApprovalPending = 3`）；审批完成时向提交人发 `ApprovalDecided = 4` |
...
|---|---|---|---|---|
| `Id` | `Guid` | `uuid` | PK | |
| `OrderType` | `SettlementOrderType` | `smallint` | NOT NULL，唯一索引 | 单据类型（复用 `023` 枚举，语义泛化为「业务单据类型」；见 `034` §0.2 同一处理） |
...
- `035` 的 `NotificationType` 续行：`ApprovalPending = 3` / `ApprovalDecided = 4`（原「逾期应收」预留值调整为 `5`，`035` 同步）。
...
| 站内信只发一次 | 提交时 + 决定时 | 不做过期升级与重复提醒（`035` 不提供），避免打扰 |
| 单据类型枚举复用 | `SettlementOrderType` | 同形枚举重复定义要避免（`030` / `034` 同一处理）；若实施时统一改名 `BusinessOrderType`，三处同步 |
