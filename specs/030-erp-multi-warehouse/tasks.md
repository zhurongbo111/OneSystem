...
> 依据 `specs/030-erp-multi-warehouse/design.md` 拆分。含**库存维度升级（破坏性）** + 仓库档案 + 五类单据带仓 + 流水带仓 + 报表 / 页面按仓改造。**按阶段顺序实现，每阶段结束跑全量测试**。
...
- [ ] 8.1 `specs/ROADMAP.md` §4.2 加「已落地（`030`）」注记（原判断与既定动作的落地结果）
