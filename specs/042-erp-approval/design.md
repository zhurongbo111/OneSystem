...
> **可用组织定位审批人（`030`）**：组织 / 部门 / 岗位 / 员工主数据由 `specs/030-erp-org-employee/` 提供，本规格的审批人可**按部门 / 岗位**圈定（如「本部门负责人」「财务岗」）作为「权限点 + 指定审批人」的补充；属**可选消费**，不改变本规格既有的简化模型（单级阈值 + 指定审批人），落地时在本节与 §0 正文一并明确。
