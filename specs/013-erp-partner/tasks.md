...
- [x] 1.3.2 `UpdatePartnerRequestHandler` 类型「只放宽不收窄」拦截：保持原类型或改为 `Both`，否则 `40119`（design §3.2 / §3.4）
- [x] 1.3.3 `PartnersController`（5 路由）+ DI 注册（仓储 + Handler + Validator）

### 1.4 错误码与 Swagger

- [x] 1.4.1 `ErrorCode.cs` 追加 40102、40119（常量 + 注释，见 design §3.2）
...
- [x] 1.5.2 `UpdatePartnerRequestHandlerTests` 补「收窄拒绝（40119）/ 放宽通过」用例
...
- [x] 2.2.1 `PartnerFormDrawer.vue`（新增 / 编辑 / 查看三态；编辑态 Name 只读——往来单位不做独立详情页，查看用抽屉 disabled 态，见 design §5；编辑态按原类型禁用会收窄的类型项并给出说明）
...
- [x] 3.1.1 `e2e/partner.spec.ts`：登录 → 建供应商 / 客户（重名 40102 提示）→ 类型筛选 → 停用后按 `status=1` 查询不含该单位 → 编辑供应商时客户项不可选、可放宽为两者
- [x] 3.1.2 `npm run test:e2e` 全绿（含既有用例回归）

## 四、交付

- [x] 4.1 规格三件套最终一致性复查（代码与 design 表结构 / 错误码 / 路由逐条对照）
- [x] 4.2 全量验证：`dotnet test` 与 `npm run e2e:run` 通过
