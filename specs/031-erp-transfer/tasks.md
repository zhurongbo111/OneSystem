...
> 依据 `specs/031-erp-transfer/design.md` 拆分。含后端新单据域（双仓库存 + 四类流水）+ 4 个用例 + 前端列表 / 开单 / 详情。按顺序实现，完成后勾选。
> 前置：`030`（多仓）已实现；`019`（流水）/ `026`（成本）已实现。
...
- [ ] 6.2 `specs/030-erp-multi-warehouse/design.md` §0 加注记：调拨产生「两条不同仓流水」的实例
- [ ] 6.3 `specs/ROADMAP.md` §4.7 单号前缀表：`TR` 行标记为已启用（`031`）
- [ ] 6.4 `specs/ROADMAP.md` §4.2 加注记：调拨（`031`）已落地；在途留待后续（与 `024` 模板统一）
- [ ] 6.5 `.codebuddy/CONTEXT.md` §2（Transfers Feature / 仓储 / 枚举）、§3（TransferManagement 域、api 文件）、§6 同步
- [ ] 6.6 `specs/ROADMAP.md` 状态列更新（`031` → 已实现）
