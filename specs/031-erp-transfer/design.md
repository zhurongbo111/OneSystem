...
> 维度与成本口径不重复定义：仓库维度见 `specs/030-erp-multi-warehouse/design.md` §0；成本见 `specs/026-erp-cost/design.md` §0.1；流水类型文案见 `specs/019-erp-stock-movement/design.md` §0。
...
| 库存约束 | 转出仓逐行 `TryDecrementAsync`，任一行不足 → 整单回滚 `40103`（message 含仓名，`030` §3.2 约定）；转入仓 `IncrementAsync` 无上限 |
...
| 单据仓库快照 | `FromWarehouseName` / `ToWarehouseName` 快照（同 `030` 单据快照口径）；列表 / 详情 / 导出免联查 |
...
- **两条流水、一个来源**：转出与转入各一条流水、指向同一调拨单，便于按仓对账（`030` §0）与按单追溯。
...
|---|---|---|---|---|
| `Id` | `Guid` | `uuid` | PK | |
| `TransferNo` | `string` | `varchar(20)` | NOT NULL，唯一索引 | 单号 `TR + yyyyMMdd + 4 位序号`（`ROADMAP` §4.7 分配 `TR`） |
...
| `BatchId` | `Guid?` | `uuid` | NULL | 批次（`032` 落地时启用；`032` 前恒为空） |
...
- 库存与流水走既有仓储（`IInventoryRepository` / `IStockMovementRepository`，均为 `030` 后的按仓签名）；跨仓储写用 `IUnitOfWork`（后端规则 §4.4）。
...
| 单号 `TR` | `ROADMAP` §4.7 分配 | 新增单据类型前先查前缀表，避免冲突（本规格落地时在表中把 `031` 行标记为已启用） |
...
- **对账一致性**：调拨 → 作废链路后，按仓 `Σ 流水 Quantity == 该仓 Inventory.Quantity`，且组织级 `Σ Quantity` 与 `Σ TotalCost` 在调拨前后不变（`030` / `026` 口径回归）。
