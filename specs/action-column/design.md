...
> 适用范围：所有表格类列表页的操作列（`a-table` 末列，`slotName: 'action'`）；页面工具条（页面级操作行）不适用。列宽策略见 `specs/list-showcase/design.md` §0；图标来源优先级见 `specs/icon-showcase/design.md` §0；行内写操作 loading 见 `specs/button-loading/design.md` §0。
...
**其他**：操作列固定显示，不参与「列设置」勾选；行内写操作 loading 按 `specs/button-loading/design.md` §0（`xxingId`），收纳进「更多」的写操作在确认入口同样按 `xxingId` 互斥（`a-doption :disabled`）。

## 1. 适用范围

- 所有表格类列表页的操作列（`a-table` 末列，`slotName: 'action'`）。
- 页面工具条（页面级操作行）不适用：其分组与分隔约定见 `specs/list-showcase/design.md` §0「页面布局」。
