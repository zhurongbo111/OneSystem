...
- [x] B2 `Program.cs` 仅 dev 环境 `AddSwaggerGen`（OpenAPI Info、双 XML 注释、Bearer 安全方案、security 逐 operation 标注）+ 管道 `UseSwagger` / `UseSwaggerUI`（置于认证/授权之前）
...
- [x] B7 新增 `SwaggerSecurityOperationFilter`（`App.Api/Swagger`）：弃用文档级 `AddSecurityRequirement`，按 `[AllowAnonymous]`（动作或控制器级）白名单语义逐 operation 标注 Bearer security（匿名接口不加）；`AddSwaggerGen` 注册 `OperationFilter<SwaggerSecurityOperationFilter>`
- [x] B8 `SwaggerIntegrationTests` 新增用例"匿名接口不带 security 要求"（`/api/auth/login` 与 `/health` 无 `security` 字段、`/api/users/me` 保留）；`dotnet build`（0 警告 0 错误）与 `dotnet test`（29 通过）全部通过
