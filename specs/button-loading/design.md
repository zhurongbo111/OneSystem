...
> 遵循 `AGENTS.md`（测试门槛 §6）与前端规则（`.codebuddy/rules/frontend/RULE.mdc` §4.6 / §5）。

## 1. 状态模型

**核心决策：loading 属于「操作」，不属于「按钮」。**

同一个异步操作可能有多个触发入口（按钮点击、输入框回车、分页器翻页、行内操作），它们必须共用同一个 loading 状态；不同操作各用独立状态。

| 操作 | 状态 | 触发入口 | 绑定 |
|---|---|---|---|
| 列表查询 | `loading` | 搜索按钮、回车、重置、刷新、翻页、每页条数 | 按钮 `:loading="loading"` + `<a-table :loading="loading">` |
| 表单提交 | `submitting` | 抽屉 / 页面表单提交按钮 | `:loading="submitting"` |
| 弹窗确认 | `xxSubmitting`（如 `resetSubmitting`） | `a-modal` 底部确定按钮 | `:ok-loading="xxSubmitting"` |
| 行内操作 | `xxingId`（如 `togglingId`） | 行内启用 / 禁用按钮 | `:loading="togglingId === row.id"` |

**为什么查询类与表格共用 `loading`**：两者描述的是同一个事实（"正在查询"）。共用后天然互斥，也避免"表格在转但按钮可点"造成用户重复提交；代价是翻页时搜索按钮也会转——语义上正确。

**为什么行内操作用行 id 而不是布尔值**：操作对象是"某一行"，用 `togglingId === row.id` 只让被点行转圈；若用整表共用的布尔值，整列按钮会一起转，用户无法判断是哪一行在处理。

## 2. 实现要求（规则 §4.6 的落地形态）

1. **触发即置位**：handler 内先置 loading，再 `await`；用 `try/finally` 复位，成功与失败路径都要复位。
2. **防重入**：动作类 handler 开头 `if (xxLoading.value) return`。
   - 必要性：Arco 的 `loading` 只表现为不可点击，**不阻止事件重入**。典型场景：搜索按钮与输入框回车是两个入口；`a-popconfirm` 的确定按钮在异步期间仍可能被再次触发。
3. **查询类不丢弃点击**：查询动作不加 `if (loading.value) return`——用户最新的筛选 / 翻页意图必须生效，正确做法是用请求序号仲裁乱序响应（`fetchSeq`），而不是丢弃新点击。丢弃会让用户"点了没反应"。
4. **不置 loading**：同步瞬时动作不置 loading——打开抽屉 / 弹窗、路由跳转、本地导出 CSV、复制文本、行内同步过滤（Mock 页面）。

## 3. 存量改造点

### 3.1 `views/LoginLogManagement/LoginLogsView.vue`
...
### 3.2 `views/UserManagement/UsersView.vue`
...
| 风险 | 处置 |
|---|---|
| 查询类按钮翻页时也会转圈，是否误导 | 语义正确（同一查询操作），且能有效阻止用户在查询中重复点搜索；如后续认为干扰，可拆分为 `queryLoading` / `tableLoading` 两个状态，但需同步改规则 §4.6 |
