...
- 关联修复：`UsersController.Me` 补 `[ProducesResponseType(401)]`（`specs/api-swagger/design.md` §2.3 已要求，此前漏实现），恢复 `SwaggerJson_dev环境_受保护接口标注401` 用例通过；后端测试由 221 通过 / 1 失败变为 **225 通过 / 0 失败**。
