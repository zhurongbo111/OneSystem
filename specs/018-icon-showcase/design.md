...
> **演进（erp-rbac）**：本页属「示例页面」分组，**不纳入权限校验**（登录即可见），无需标注权限点。判据见 `specs/028-erp-rbac/design.md` §0.1。
> **演进（erp-audit-log）**：本规格为纯前端示例页（无后端写路径），不涉及操作日志。
...
- **Arco 组件 `#icon` 插槽**（`a-button` / `a-menu-item` / `a-select` 等，含 `specs/011-action-column/design.md` §0 的操作列固定图标）**同样优先 Tabler**，三套均可放进插槽；仅当语义只有 Arco 覆盖时才用 Arco。
