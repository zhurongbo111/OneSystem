...
> 判定规则不重复定义：低库存见 `specs/030-erp-multi-warehouse/design.md` §0；近效期 / 过期见 `specs/032-erp-batch-expiry/design.md` §0。

## 0. 告警信号约定（唯一事实源）

| 信号类型（`AlertType`） | 判定（只读查询） | 跳转目标 |
|---|---|---|
| `LowStock = 0` 低库存 | 按「商品 × 仓」汇总：`SUM(Quantity) < MAX(SafetyStock)` 且 `MAX(SafetyStock) > 0` 且商品启用（`030` §0 口径） | `/inventory?warehouseId=&keyword=<商品编码>` |
| `ExpiringBatch = 1` 近效期 | 批次未过期且 `ExpiryDate <= 今天 + 30`（`BatchFieldConstraints.NearExpiryDays`）且该 `(商品, 仓, 批次)` 库存 > 0（`032` §0 口径） | `/batches?productId=&keyword=<批次号>` |
...
- **扫描与站内信解耦**：扫描器产出「信号 + 接收人 + 内容」，通过 `INotificationWriter` 落库；站内信能力（`035`）同时服务后续通知（如 `036` 的待审批提醒）。
...
| `Type` | `NotificationType` | `smallint` | NOT NULL | 0 = 低库存 1 = 近效期 2 = 过期 3 = 待审批（`036` 续行）4 = 审批结果（`036` 续行）5 = 逾期应收（预留） |
...
- `App.Core/Entities/NotificationType.cs`（§2.1 取值）、`App.Core/Entities/AlertType.cs`（0–2，与前三者一致）；`036` 追加 `NotificationType.ApprovalPending = 3` 与 `ApprovalDecided = 4`（逾期应收预留值随之改为 5；追加枚举值不涉及迁移）。
...
| `AlertType` 与 `NotificationType` 取值对齐（前三类） | 两个枚举 | 台账只关心前几类信号；未来若新增「非告警类通知」（如 `036` 的待审批）只需扩 `NotificationType`，不影响去重台账 |
