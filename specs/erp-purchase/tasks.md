...
- [x] 1.2.2 仓储构造函数注入 `ICurrentUser` 填充审计字段（同 `UserRepository` 模式）；跨仓储写在 Handler 中用 `IUnitOfWork` 事务
...
- [x] 2.4.2 手动联调走查：开采购单（供应商 + 2 行明细，改一行单价）→ 保存跳详情（单号 / 总额正确）→ 库存增加 → 标记已付 → 作废回冲 → 作废后操作消失，全链路无 console 报错
