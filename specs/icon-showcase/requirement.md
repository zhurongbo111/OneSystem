...
前端组件示例页（`frontend-component-showcase`）集中展示各图标库的用法，作为业务页面选图标的参考。涉及三套图标库：Arco Design 自带图标（`@arco-design/web-vue` 的 `Icon*`，业务默认使用）、开源图标集 Tabler Icons（`@tabler/icons-vue`）与 Lucide Icons（`@lucide/vue`，均为线性风格）。三者在示例页「图标」tab 内分组展示，供对照选用。
