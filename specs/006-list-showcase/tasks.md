...
- [x] T15 列设置补齐到销售开单 / 采购入库页（此前两页无列设置）：`columns` 由静态常量改为 computed 按 `visibleColumns` 拼列（§2.4），序号与操作列固定显示、不参与设置（specs/011-action-column §5），补 e2e 断言，重新验证 build/lint/e2e
