...
> 适用范围：所有表格类列表页。操作列细则见 `specs/011-action-column/design.md` §0；按钮 loading 见 `specs/010-button-loading/design.md` §0；图标选型见 `specs/018-icon-showcase/design.md` §0。
...
- 查询类按钮的 loading 绑定见 `specs/010-button-loading/design.md` §0（与表格共用同一个 `loading`）。
...
- 图标一律取 Tabler，选型优先级与映射见 `specs/018-icon-showcase/design.md` §0；同一操作全项目同一图标（重置用 `IconRestore`，不复用已归属「刷新」的 `IconRefresh`）。
...
- 操作列形态（平铺 / 更多 / 危险样式）断言见 `specs/011-action-column/`。
