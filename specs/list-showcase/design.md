...
> **通用布局与交互约定**（页面头只放标题、工具条两行与左右分组、表格配置、列宽策略、请求序号仲裁、CSV BOM）的唯一事实源是前端规则 `.codebuddy/rules/frontend/RULE.mdc` §5；本规格只描述这个参照页面自身的内容。
...
自上而下（布局约定见前端规则 §5「页面布局」，此处只列本页内容）：
...
- 图标一律取 Tabler，选型优先级与映射见前端规则 §4.7；同一操作全项目同一图标（重置用 `IconRestore`，不复用已归属「刷新」的 `IconRefresh`）。
...
- 其余表格配置（`row-key` 必设、分页 `showTotal` + `showPageSize` + `pageSizeOptions`、每列固定 `width` + `:scroll="{ x: tableScrollX }"`）见前端规则 §5「表格配置 / 列宽策略」；本页 `pageSize = 10`。
...
- `exportCsv()`：取**当前已应用筛选 + 排序**的全量结果（不分页），按可见列拼接 CSV；表头加 BOM（见前端规则 §5）；拼接 `Blob` → `URL.createObjectURL` → `<a download="用户列表.csv">`。
...
- 筛选条件输入态（`keywordInput` / `statusInput` / `roleInput`）与已应用态（`applied*`）分离，点「搜索」应用并回第 1 页（模式见前端规则 §5「交互模式」）。
