...
> 依据 `specs/041-erp-stock-alert/design.md` 拆分。含后端扫描能力 + 定时宿主 + 5 个用例 + 前端顶栏铃铛与站内信列表。**先做扫描与站内信，再做宿主**（宿主最简单且最难测，放最后）。
> 前置：`028`（权限点 `inventory.view`、`IPermissionResolver`）与 `038`（仓级安全库存）已实现；近效期 / 过期信号依赖 `040`（若 `040` 未落地，阶段 B 可延后）。
...
- [ ] 2.1 `INotificationWriter` + 实现（批量写站内信，供扫描器与 `042` 复用）
...
- [ ] 8.1 `specs/038-erp-multi-warehouse/design.md` §0 加注记：低库存告警消费仓级阈值口径
- [ ] 8.2 `specs/040-erp-batch-expiry/design.md` §0 加注记：近效期 / 过期告警消费 `NearExpiryDays` 与过期判定
- [ ] 8.3 `specs/028-erp-rbac/design.md` §0.2 续行：`notifications.scan`
- [ ] 8.4 `specs/036-erp-partner-price/design.md` §5 加注记：催收提醒所需的站内信通道已由 `041` 交付（规则后续）
- [ ] 8.5 `specs/042-erp-approval/` 引用 `INotificationWriter` 生成待审批通知（在 `042` tasks 联动）
...
- [ ] 8.7 `specs/ROADMAP.md` 状态列更新（`041` → 已实现）
