...
> **演进（erp-rbac）**：菜单项与路由接入权限过滤——菜单项追加 `permission`（未命中则隐藏，分组内无可见子项则整组隐藏），路由 `meta.permission` 由守卫校验（未登录跳登录页、无权限跳 `/403`；`403` 为顶层路由，无侧边栏 / 工具条）。权限点清单与白名单唯一来源见 `specs/028-erp-rbac/design.md` §0.2 / §0.4。
