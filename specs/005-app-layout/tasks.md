...
- [x] G1 `specs/005-app-layout/requirement.md` / `design.md` 更新：侧边菜单「组件示例 / 列表示例 / 表单与详情示例」收纳进「示例页面」`a-sub-menu`（默认展开，`open-keys` 与 `@open-change` 联动）
...
- [x] H1 `specs/005-app-layout/requirement.md` / `design.md` 更新：子菜单默认折叠（`openKeys` 初始为空），仅当前路由所属分组自动展开（`watch(route.name, { immediate: true })` 只增不减）
