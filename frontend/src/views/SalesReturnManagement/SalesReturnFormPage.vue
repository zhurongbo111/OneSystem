...
import { createSalesReturn, toUtcMidnight } from '@/api/saleReturn'
...
const lines = ref<SalesReturnFormLine[]>([newLine()])
...
function onLineProductChange(line: SalesReturnFormLine, value?: string): void {
...
function onLineUnitPriceChange(line: SalesReturnFormLine, value: number | undefined): void {
...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    void router.push({ name: 'saleReturnDetail', params: { id: saved.id } })
  } catch {
    // 错误提示已由请求层统一处理
  } finally {
      ...
      >
        ...
        >
          ...
          <template #quantity="{ record }">
            <a-input-number
              :model-value="(record as SalesReturnFormLine).quantity"
              :min="QUANTITY_MIN"
              :max="QUANTITY_MAX"
              style="width: 100%"
              @change="(v: number | undefined) => onLineQuantityChange(record as SalesReturnFormLine, v)"
            />
          </template>
          <template #unitPrice="{ record }">
            <a-input-number
              :model-value="(record as SalesReturnFormLine).unitPrice"
              :min="0"
              :precision="2"
              prefix="¥"
              style="width: 100%"
              @change="(v: number | undefined) => onLineUnitPriceChange(record as SalesReturnFormLine, v)"
            />
          </template>
          <template #subtotal="{ record }">
            <span class="line-subtotal">
              {{ ((record as SalesReturnFormLine).quantity * (record as SalesReturnFormLine).unitPrice).toFixed(2) }}
            </span>
          </template>
          <template #itemAction="{ record }">
            <a-button
              ...
              status="danger"
              size="small"
              @click="removeItem((record as SalesReturnFormLine).key)"
            >
              删除
            </a-button>
          </template>
        ...
        <a-form-item label="备注">
          <a-textarea
            v-model="remark"
            placeholder="选填（不超过 200 字，可写原销售单号 / 退货原因）"
            :max-length="200"
