    ...
    >
      ...
      >
        ...
        <template #settlement="{ record }">
          <a-tag :color="(record as SalesOrderListItem).settlementStatus === 1 ? 'green' : 'orange'">
            {{ (record as SalesOrderListItem).settlementStatus === 1 ? '已收' : '未收' }}
          </a-tag>
        </template>
        <template #status="{ record }">
          <a-tag :color="(record as SalesOrderListItem).status === 1 ? 'green' : 'gray'">
        ...
        <template #action="{ record }">
          ...
          >
            ...
            <a-popconfirm
              v-if="(record as SalesOrderListItem).status === 1"
              type="info"
              :content="(record as SalesOrderListItem).settlementStatus === 1 ? '确认改回未收？' : '确认标记为已收？'"
