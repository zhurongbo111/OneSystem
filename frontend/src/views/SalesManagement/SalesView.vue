...
import { exportSalesShipments } from '@/api/export'
...
/** 关键词 / 客户 / 出库仓 / 日期范围 / 结算：输入态与已应用态分离（点搜索才生效） */
...
/** 导出当前已应用筛选的全量销售出库单（单据 + 明细两个工作表）；失败提示由请求层统一处理 */
...
function onPrint(row: SalesShipmentListItem): void {
  void router.push({ name: 'salePrint', params: { id: row.id } })
}
  ...
  <div class="list-page">
    ...
    >
      ...
      >
        ...
        <!-- 操作列（specs/011-action-column §0）：4 个操作 > 3，平铺 详情/收付款/作废，「打印」收纳进「更多」；详情恒显，收付款/作废仅正常单显示 -->
        <template #action="{ record }">
          ...
          >
            ...
            <a-button
              v-if="canStartSettlement(record as SalesShipmentListItem)"
              type="text"
              size="small"
            ...
            <a-tooltip
              v-if="(record as SalesShipmentListItem).status === 1 && !canVoidOrder(record as SalesShipmentListItem)"
            ...
            <a-popconfirm
              v-else-if="canVoidOrder(record as SalesShipmentListItem)"
              type="warning"
            ...
            <a-dropdown trigger="click">
              ...
              <template #content>
                <a-doption
                  value="print"
                  @click="onPrint(record as SalesShipmentListItem)"
                >
                  <template #icon>
                    <IconPrinter />
                  </template>
