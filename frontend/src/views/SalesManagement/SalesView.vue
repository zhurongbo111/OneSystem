...
/** 结算切换：未结算 ↔ 已结算（库存不变） */
async function onToggleSettlement(row: SalesOrderListItem): Promise<void> {
  if (settlingId.value) return
  settlingId.value = row.id
  try {
    const next: SettlementStatus = row.settlementStatus === 1 ? 0 : 1
    await updateSalesOrderSettlement(row.id, next)
    ...
    >
      ...
      >
        ...
        <template #settlement="{ record }">
          <a-tag :color="(record as SalesOrderListItem).settlementStatus === 1 ? 'green' : 'gray'">
            {{ (record as SalesOrderListItem).settlementStatus === 1 ? '已结算' : '未结算' }}
          </a-tag>
        </template>
        <template #status="{ record }">
          <a-tag :color="(record as SalesOrderListItem).status === 1 ? 'green' : 'red'">
            {{ (record as SalesOrderListItem).status === 1 ? '正常' : '已作废' }}
          </a-tag>
        </template>
        ...
        <template #action="{ record }">
          ...
          >
            ...
            <a-popconfirm
              v-if="(record as SalesOrderListItem).status === 1"
              type="info"
              :content="(record as SalesOrderListItem).settlementStatus === 1 ? '确认改回未结算？' : '确认标记为已结算？'"
              @ok="onToggleSettlement(record as SalesOrderListItem)"
            >
              <a-button
                type="text"
              ...
              >
                ...
                {{ (record as SalesOrderListItem).settlementStatus === 1 ? '改回未结算' : '标记已结算' }}
              </a-button>
            </a-popconfirm>
            <a-popconfirm
              v-if="(record as SalesOrderListItem).status === 1"
              ...
              @ok="onVoid(record as SalesOrderListItem)"
            >
              <a-button
                type="text"
                status="danger"
