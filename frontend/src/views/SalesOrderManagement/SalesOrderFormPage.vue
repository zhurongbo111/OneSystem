...
import { toUtcMidnight } from '@/api/sale'
import { createSalesOrder, getSalesOrder, updateSalesOrder } from '@/api/saleOrder'
import type { SalesOrderFormLine } from '@/api/saleOrder'
...
function newLine(): SalesOrderFormLine {
...
const lines = ref<SalesOrderFormLine[]>([newLine()])
...
function onLineProductChange(line: SalesOrderFormLine, value?: string): void {
...
function onLineQuantityChange(line: SalesOrderFormLine, value: number | undefined): void {
  line.quantity = value ?? 0
}

function onLineUnitPriceChange(line: SalesOrderFormLine, value: number | undefined): void {
...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    Message.success(isEdit.value ? '销售订单已保存' : '销售订单已创建')
    void router.push({ name: 'salesOrderDetail', params: { id: saved.id } })
  } catch {
    // 错误提示已由请求层统一处理
  } finally {
      ...
      >
        ...
        >
          ...
          <template #product="{ record }">
            <a-select
              :model-value="(record as SalesOrderFormLine).productId"
              :options="productOptions"
              placeholder="请选择商品"
              allow-search
              allow-clear
              @change="(v: string | number | boolean | Record<string, unknown> | (string | number | boolean | Record<string, unknown>)[]) => onLineProductChange(record as SalesOrderFormLine, v as string | undefined)"
            />
          </template>
          <template #quantity="{ record }">
            <a-input-number
              :model-value="(record as SalesOrderFormLine).quantity"
              :min="QUANTITY_MIN"
              :max="QUANTITY_MAX"
              style="width: 100%"
              @change="(v: number | undefined) => onLineQuantityChange(record as SalesOrderFormLine, v)"
            />
          </template>
          <template #unitPrice="{ record }">
            <a-input-number
              :model-value="(record as SalesOrderFormLine).unitPrice"
              :min="0"
              :precision="2"
              prefix="¥"
              style="width: 100%"
              @change="(v: number | undefined) => onLineUnitPriceChange(record as SalesOrderFormLine, v)"
            />
          </template>
          <template #subtotal="{ record }">
            <span class="line-subtotal">
              {{ ((record as SalesOrderFormLine).quantity * (record as SalesOrderFormLine).unitPrice).toFixed(2) }}
            </span>
          </template>
          <template #itemAction="{ record }">
            <a-button
              ...
              status="danger"
              size="small"
              @click="removeItem((record as SalesOrderFormLine).key)"
            >
              删除
            </a-button>
          </template>
