...
import { getWarehouses, setDefaultWarehouse, updateWarehouseStatus } from '@/api/warehouse'
import type { Warehouse, WarehouseStatus } from '@/api/warehouse'
...
import WarehouseFormDrawer from './WarehouseFormDrawer.vue'
...
/** 正在启停的仓库 id：行内按钮 loading 与写操作互斥用 */
const togglingId = ref<string | undefined>(undefined)
/** 正在设为默认的仓库 id（与启停分开，避免两个操作互相干扰） */
const settingDefaultId = ref<string | undefined>(undefined)
const items = ref<Warehouse[]>([])
const total = ref(0)
const page = ref(1)
const pageSize = ref(20)

/** 关键词 / 状态：输入态与已应用态分离（点搜索才生效） */
const keywordInput = ref('')
const statusInput = ref<WarehouseStatus | undefined>(undefined)
const appliedKeyword = ref('')
const appliedStatus = ref<WarehouseStatus | undefined>(undefined)
...
const columns = computed<TableColumnData[]>(() => {
  ...
  if (visibleColumns.value.includes('name')) {
    cols.push({ title: '名称', slotName: 'name', width: 200, ellipsis: true, tooltip: true })
  }
  ...
  if (visibleColumns.value.includes('contact')) {
    cols.push({ title: '联系人', dataIndex: 'contact', width: 110 })
  }
  if (visibleColumns.value.includes('phone')) {
    cols.push({ title: '电话', dataIndex: 'phone', width: 140 })
  }
  ...
  // 操作列：3 个操作 ≤ 3 平铺（编辑 / 设为默认 / 停用或启用），宽度按 specs/011-action-column §0 参考取 220
  cols.push({ title: '操作', slotName: 'action', width: 220, bodyCellClass: 'action-cell' })
  return cols
})
...
/** 编辑（抽屉） */
function onEdit(row: Warehouse): void {
  drawerMode.value = 'edit'
  drawerEditId.value = row.id
  drawerVisible.value = true
}

/** 启用 / 停用（默认仓由后端拒绝，返回 40124） */
async function onToggleStatus(row: Warehouse): Promise<void> {
  if (togglingId.value) return
  const next: WarehouseStatus = row.status === 1 ? 0 : 1
  togglingId.value = row.id
  try {
    await updateWarehouseStatus(row.id, next)
  ...
  <div class="list-page">
    ...
    >
      <div class="toolbar">
        ...
        <!-- 操作行：左组主操作（新增）靠左，右组视图操作（列设置 / 刷新）靠右 -->
        <div class="toolbar-actions">
          <div class="toolbar-actions__left">
            <a-button
              v-if="auth.hasPermission('warehouses.create')"
              type="primary"
              size="small"
              @click="onCreate"
            >
      ...
      >
        ...
        <template #name="{ record }">
          <span class="warehouse-name">{{ (record as Warehouse).name }}</span>
          <a-tag
            v-if="(record as Warehouse).isDefault"
            color="blue"
            size="small"
          >
            默认
        ...
        <template #status="{ record }">
          <a-tag :color="(record as Warehouse).status === 1 ? 'green' : 'red'">
            {{ (record as Warehouse).status === 1 ? '启用' : '停用' }}
          </a-tag>
        </template>
        <template #createdAt="{ record }">
          {{ formatDateTime((record as Warehouse).createdAt) }}
        </template>
        <!-- 操作列（specs/011-action-column §0）：3 个操作 ≤ 3 平铺 编辑 / 设为默认 / 停用或启用 -->
        <template #action="{ record }">
          ...
          >
            <a-button
              type="text"
              size="small"
              @click="onEdit(record as Warehouse)"
            >
              <template #icon>
                <IconEdit />
              </template>
            ...
            <a-button
              v-if="!(record as Warehouse).isDefault && (record as Warehouse).status === 1"
              type="text"
              status="success"
              size="small"
              :loading="settingDefaultId === (record as Warehouse).id"
              @click="onSetDefault(record as Warehouse)"
            >
              <template #icon>
                <IconStar />
              </template>
            ...
            <a-popconfirm
              type="warning"
              :content="`确认${(record as Warehouse).status === 1 ? '停用' : '启用'}该仓库？`"
              @ok="onToggleStatus(record as Warehouse)"
            >
              <a-button
                type="text"
                :status="(record as Warehouse).status === 1 ? 'warning' : 'normal'"
                size="small"
                :loading="togglingId === (record as Warehouse).id"
              >
                <template #icon>
                  <IconPower v-if="(record as Warehouse).status === 1" />
                  <IconPlayerPlay v-else />
                </template>
                {{ (record as Warehouse).status === 1 ? '停用' : '启用' }}
              </a-button>
            </a-popconfirm>
          </a-space>
        </template>
