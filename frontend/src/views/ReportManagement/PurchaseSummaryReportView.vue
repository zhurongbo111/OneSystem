...
import { exportPurchaseSummary, getPurchaseSummary, toReportRangeUtc } from '@/api/report'
import type { PurchaseSummaryItem, PurchaseSummaryTotal, SummaryGroupBy } from '@/api/report'
...
const items = ref<PurchaseSummaryItem[]>([])
const total = ref(0)
const page = ref(1)
const pageSize = ref(20)
const summary = ref<PurchaseSummaryTotal>({
  orderCount: 0,
  inboundQuantity: 0,
  inboundAmount: 0,
  returnQuantity: 0,
...
const columns = computed<TableColumnData[]>(() => {
  const nameTitle = appliedGroupBy.value === 'product' ? '商品' : '供应商'
  ...
  cols.push(
    { title: '入库单数', dataIndex: 'orderCount', width: 100, align: 'right' },
    { title: '入库数量', dataIndex: 'inboundQuantity', width: 110, align: 'right' },
    { title: '入库金额', slotName: 'inboundAmount', width: 130, align: 'right' },
...
async function fetchPartners(): Promise<void> {
  try {
    const result = await getPartners({ type: 1, status: 1, page: 1, pageSize: 100 })
...
/** 导出当前已应用筛选的全量采购汇总（含合计行）；失败提示由请求层统一处理 */
async function onExport(): Promise<void> {
  exporting.value = true
  try {
    const { start, end } = toReportRangeUtc(appliedRange.value[0], appliedRange.value[1])
    await exportPurchaseSummary({
      start,
      end,
      partnerId: appliedPartnerId.value,
      groupBy: appliedGroupBy.value,
  ...
  <div class="list-page">
    ...
    >
      ...
      >
        ...
        <template #inboundAmount="{ record }">
          {{ formatMoney((record as PurchaseSummaryItem).inboundAmount) }}
        </template>
        <template #returnAmount="{ record }">
          {{ formatMoney((record as PurchaseSummaryItem).returnAmount) }}
        </template>
        <template #netQuantity="{ record }">
          <span class="net-value">{{ (record as PurchaseSummaryItem).netQuantity }}</span>
        </template>
        <template #netAmount="{ record }">
          <span class="net-value">{{ formatMoney((record as PurchaseSummaryItem).netAmount) }}</span>
        </template>
      </a-table>
    </a-card>
  </div>
