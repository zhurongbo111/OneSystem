...
import { exportPurchaseReceipts } from '@/api/export'
...
/** 导出当前已应用筛选的全量采购入库单（单据 + 明细两个工作表）；失败提示由请求层统一处理 */
...
function onPrint(row: PurchaseReceiptListItem): void {
  void router.push({ name: 'purchasePrint', params: { id: row.id } })
}
  ...
  <div class="list-page">
    ...
    >
      <div class="toolbar">
        ...
        <!-- 操作行：左组主操作（开采购单）靠左，右组视图操作（列设置/刷新）靠右，同一行 -->
      ...
      >
        ...
        <!-- 操作列（specs/011-action-column §0）：4 个操作 > 3，平铺 详情/收付款/作废，「打印」收纳进「更多」；详情恒显，收付款/作废仅正常单显示（design §4.4） -->
        <template #action="{ record }">
          ...
          >
            ...
            <a-button
              v-if="canStartSettlement(record as PurchaseReceiptListItem)"
              type="text"
              size="small"
            ...
            <a-tooltip
              v-if="(record as PurchaseReceiptListItem).status === 1 && !canVoidOrder(record as PurchaseReceiptListItem)"
            ...
            <a-popconfirm
              v-else-if="canVoidOrder(record as PurchaseReceiptListItem)"
              type="warning"
            ...
            <a-dropdown trigger="click">
              ...
              <template #content>
                <a-doption
                  value="print"
                  @click="onPrint(record as PurchaseReceiptListItem)"
                >
                  <template #icon>
                    <IconPrinter />
                  </template>
