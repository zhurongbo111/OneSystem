...
import { exportPurchaseReturns } from '@/api/export'
...
/** 关键词 / 供应商 / 出库仓 / 日期范围 / 结算：输入态与已应用态分离（点搜索才生效） */
...
const columns = computed<TableColumnData[]>(() => {
  ...
  if (visibleColumns.value.includes('warehouseName')) {
    // 出库仓（038）：单据保存即固化，此处只读展示
    cols.push({ title: '出库仓', dataIndex: 'warehouseName', width: 140, ellipsis: true, tooltip: true })
  }
...
function onCreate(): void {
  void router.push({ name: 'purchaseReturnNew' })
}

function onDetail(row: PurchaseReturnListItem): void {
  void router.push({ name: 'purchaseReturnDetail', params: { id: row.id } })
}

/** 导出当前已应用筛选的全量采购退货单（单据 + 明细两个工作表）；失败提示由请求层统一处理 */
...
function onPrint(row: PurchaseReturnListItem): void {
  void router.push({ name: 'purchaseReturnPrint', params: { id: row.id } })
}

/** 作废行整体置灰（design §4.4） */
function rowClassName(record: PurchaseReturnListItem): string {
  return record.status === 0 ? 'row-voided' : ''
}

/** 作废：回冲库存，仅改状态不删数据 */
async function onVoid(row: PurchaseReturnListItem): Promise<void> {
  if (voidingId.value) return
  voidingId.value = row.id
  try {
    await voidPurchaseReturn(row.id)
    ...
    >
      <div class="toolbar">
        ...
        <div class="toolbar-actions">
          <div class="toolbar-actions__left">
            <a-button
              v-if="auth.hasPermission('purchaseReturns.create')"
              type="primary"
              size="small"
              @click="onCreate"
            >
      ...
      >
        ...
        <template #returnNo="{ record }">
          {{ (record as PurchaseReturnListItem).returnNo }}
        </template>
        <template #returnDate="{ record }">
          {{ formatDateTime((record as PurchaseReturnListItem).returnDate).slice(0, 10) }}
        </template>
        <template #totalAmount="{ record }">
          <span class="amount">
            ¥ {{ (record as PurchaseReturnListItem).totalAmount.toFixed(2) }}
          </span>
        </template>
        ...
        <template #status="{ record }">
          <a-tag :color="(record as PurchaseReturnListItem).status === 1 ? 'green' : 'red'">
            {{ (record as PurchaseReturnListItem).status === 1 ? '正常' : '已作废' }}
          </a-tag>
        </template>
        <template #createdAt="{ record }">
          {{ formatDateTime((record as PurchaseReturnListItem).createdAt) }}
        </template>
        <!-- 操作列（specs/011-action-column §0）：4 个操作 > 3，平铺 详情/收付款/作废，「打印」收纳进「更多」；详情恒显，作废/收付款仅正常单显示（design §4.4） -->
        <template #action="{ record }">
          ...
          >
            <a-button
              type="text"
              size="small"
              @click="onDetail(record as PurchaseReturnListItem)"
            >
              <template #icon>
                <IconEye />
              </template>
            ...
            <a-button
              v-if="canStartSettlement(record as PurchaseReturnListItem)"
              type="text"
              size="small"
            ...
            <a-tooltip
              v-if="(record as PurchaseReturnListItem).status === 1 && !canVoidOrder(record as PurchaseReturnListItem)"
            ...
            <a-popconfirm
              v-else-if="canVoidOrder(record as PurchaseReturnListItem)"
              type="warning"
              content="确认作废该采购退货单？作废后库存将回冲，且不可恢复"
              @ok="onVoid(record as PurchaseReturnListItem)"
            >
              <a-button
                type="text"
                size="small"
                status="danger"
                :loading="voidingId === (record as PurchaseReturnListItem).id"
              >
                <template #icon>
                  <IconBan />
                </template>
            ...
            <a-dropdown trigger="click">
              ...
              <template #content>
                <a-doption
                  value="print"
                  @click="onPrint(record as PurchaseReturnListItem)"
                >
                  <template #icon>
                    <IconPrinter />
                  </template>
