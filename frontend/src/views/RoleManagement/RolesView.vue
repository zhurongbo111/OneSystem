...
import { IconEdit, IconPlus, IconRefresh, IconRestore, IconSearch, IconTrash } from '@tabler/icons-vue'
...
/** 正在删除的角色 id：行内按钮 loading 与写操作互斥用 */
...
const columns = computed<TableColumnData[]>(() => [
  ...
  { title: '类型', slotName: 'isBuiltin', width: 100 },
  { title: '权限数', dataIndex: 'permissionCount', width: 90, align: 'right' },
  { title: '用户数', dataIndex: 'userCount', width: 90, align: 'right' },
  { title: '备注', dataIndex: 'remark', ellipsis: true, tooltip: true },
  { title: '创建时间', dataIndex: 'createdAt', width: 172, slotName: 'createdAt' },
  // 列宽按实测内容 160px（2×66 文本按钮 + 2×8 padding + 间距）
...
/** 编辑（抽屉） */
function onEdit(row: RoleListItem): void {
  drawerMode.value = 'edit'
  drawerEditId.value = row.id
  drawerVisible.value = true
}

/** 删除（内置角色不可删，由后端 40175 兜底，此处仅禁用入口） */
async function onDelete(row: RoleListItem): Promise<void> {
  ...
  try {
    ...
    // 错误提示已由请求层统一处理（40174 有用户绑定 / 40175 内置角色）
  } finally {
    ...
    >
      ...
      >
        ...
        <template #isBuiltin="{ record }">
          <a-tag :color="(record as RoleListItem).isBuiltin ? 'blue' : 'gray'">
            {{ (record as RoleListItem).isBuiltin ? '内置' : '自定义' }}
          </a-tag>
        </template>
        <template #createdAt="{ record }">
          {{ formatDateTime((record as RoleListItem).createdAt) }}
        </template>
        ...
        <template #action="{ record }">
          ...
          >
            <a-button
              ...
              type="text"
              size="small"
              @click="onEdit(record as RoleListItem)"
            >
              <template #icon>
                <IconEdit />
              </template>
            ...
            <a-popconfirm
              type="warning"
              :disabled="(record as RoleListItem).isBuiltin"
              :content="`确认删除角色「${(record as RoleListItem).name}」？`"
              @ok="onDelete(record as RoleListItem)"
            >
              <a-button
                ...
                size="small"
                :disabled="(record as RoleListItem).isBuiltin"
                :loading="deletingId === (record as RoleListItem).id"
              >
                <template #icon>
                  <IconTrash />
                </template>
...
/* 操作列密度（specs/011-action-column §5）：收窄文本按钮默认水平 padding */
.row-actions :deep(.arco-btn-text) {
  padding: 0 8px;
}

/* 操作列兜底（specs/011-action-column §5）：按钮组不折行 */
:deep(.action-cell) {
  white-space: nowrap;
}
