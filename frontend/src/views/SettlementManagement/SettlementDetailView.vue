...
async function onVoid(): Promise<void> {
  ...
  try {
    await voidSettlement(detail.value.id)
    Message.success('已作废，单据已结算金额已回退')
    detail.value = await getSettlement(detail.value.id)
  } catch {
    // 错误提示已由请求层统一处理
  } finally {
      ...
      <a-card :bordered="false">
        ...
        >
          ...
          <a-descriptions-item label="类型">
            <a-tag :color="detail.type === 0 ? 'green' : 'orange'">
              {{ detail.type === 0 ? '收款' : '付款' }}
            </a-tag>
          </a-descriptions-item>
        ...
        >
          ...
          <template #orderTotalAmount="{ record }">
            ¥ {{ (record as SettlementItem).orderTotalAmount.toFixed(2) }}
          </template>
          <template #amount="{ record }">
            ¥ {{ (record as SettlementItem).amount.toFixed(2) }}
          </template>
        </a-table>
      </a-card>
