...
import { deletePosition, getPositions, updatePositionStatus } from '@/api/position'
import type { Position, PositionStatus } from '@/api/position'
...
import PositionFormDrawer from './PositionFormDrawer.vue'
...
/** 正在启停的岗位 id：行内按钮 loading 与写操作互斥用 */
...
const statusInput = ref<PositionStatus | undefined>(undefined)
const appliedKeyword = ref('')
const appliedStatus = ref<PositionStatus | undefined>(undefined)
...
const columns: TableColumnData[] = [
  ...
  { title: '岗位名称', dataIndex: 'name', width: 180, ellipsis: true, tooltip: true },
...
/** 编辑（抽屉） */
function onEdit(row: Position): void {
...
/** 启用 / 停用 */
async function onToggleStatus(row: Position): Promise<void> {
  if (togglingId.value) return
  const next: PositionStatus = row.status === 1 ? 0 : 1
  togglingId.value = row.id
  try {
    await updatePositionStatus(row.id, next)
...
/** 删除（被员工引用时后端拒绝并提示 40144） */
async function onDelete(row: Position): Promise<void> {
    ...
    >
      <div class="toolbar">
        ...
        <div class="toolbar-actions">
          <div class="toolbar-actions__left">
            <a-button
              v-if="auth.hasPermission('positions.create')"
              type="primary"
              size="small"
              @click="onCreate"
            >
      ...
      >
        ...
        <template #status="{ record }">
          <a-tag :color="(record as Position).status === 1 ? 'green' : 'red'">
            {{ (record as Position).status === 1 ? '启用' : '停用' }}
          </a-tag>
        </template>
        <template #createdAt="{ record }">
          {{ formatDateTime((record as Position).createdAt) }}
        </template>
        ...
        <template #action="{ record }">
          ...
          >
            <a-button
              v-if="auth.hasPermission('positions.update')"
              type="text"
              size="small"
              @click="onEdit(record as Position)"
            >
              <template #icon>
                <IconEdit />
              </template>
            ...
            <a-popconfirm
              v-if="auth.hasPermission('positions.status')"
              type="warning"
              :content="`确认${(record as Position).status === 1 ? '停用' : '启用'}该岗位？`"
              @ok="onToggleStatus(record as Position)"
            >
              <a-button
                type="text"
                :status="(record as Position).status === 1 ? 'warning' : 'normal'"
                size="small"
                :loading="togglingId === (record as Position).id"
              >
                <template #icon>
                  <IconPower v-if="(record as Position).status === 1" />
                  <IconPlayerPlay v-else />
                </template>
                {{ (record as Position).status === 1 ? '停用' : '启用' }}
              </a-button>
            </a-popconfirm>
            <a-popconfirm
              v-if="auth.hasPermission('positions.delete')"
              type="warning"
              content="确认删除该岗位？被员工引用时不可删除"
              @ok="onDelete(record as Position)"
            >
              <a-button
                type="text"
                status="danger"
                size="small"
                :loading="deletingId === (record as Position).id"
              >
                <template #icon>
                  <IconTrash />
                </template>
