...
async function searchMovementByOrderNo(page: Page, orderNo: string): Promise<void> {
  ...
  await page.getByRole('button', { name: '搜索', exact: true }).click()
  await expect(dataRows(page).first()).toContainText(orderNo, { timeout: 15000 })
}

/** 在退货列表按单号搜索（等到过滤后首行命中该单号） */
async function searchReturnByNo(page: Page, returnNo: string): Promise<void> {
  ...
  await page.getByRole('button', { name: '搜索', exact: true }).click()
  await expect(dataRows(page).first()).toContainText(returnNo, { timeout: 15000 })
}
