...
async function goPurchases(page: Page): Promise<void> {
  await login(page)
  await page.locator('.arco-menu-item', { hasText: '采购入库' }).click()
...
test.describe('采购入库（集成）', () => {
  ...
  test('开单 → 库存增加 → 标记已付 → 作废回冲（全链路）', async ({ page }) => {
    ...
    await confirmPopconfirm(page, '确认标记为已付？')
    ...
    await expect(rowVoided.getByRole('button', { name: '标记已付' })).toHaveCount(0)
    await expect(rowVoided.getByRole('button', { name: '改回未付' })).toHaveCount(0)
