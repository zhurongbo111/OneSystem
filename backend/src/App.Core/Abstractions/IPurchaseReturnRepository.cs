...
public interface IPurchaseReturnRepository
{
    ...
    /// <param name="warehouseId">出库仓 id，可空（038；不传 = 全部仓）</param>
    ...
    /// <summary>
    /// 查询采购退货单详情（主表实体 + 可选明细行实体，按明细 Id 还原插入顺序），不存在时 Return 为 null
    /// </summary>
    ...
    /// </param>
    /// <param name="cancellationToken">取消令牌</param>
    Task<(PurchaseReturn? Return, IReadOnlyList<PurchaseReturnItem> Items)> GetDetailAsync(Guid id, bool includeItems = true, CancellationToken cancellationToken = default);
