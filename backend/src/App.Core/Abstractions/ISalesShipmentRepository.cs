...
public interface ISalesShipmentRepository
{
    ...
    /// <summary>
    /// 查询销售单详情（主表实体 + 可选明细行实体，按明细 Id 还原插入顺序），不存在时 Order 为 null
    /// </summary>
    ...
    /// </param>
    /// <param name="cancellationToken">取消令牌</param>
    Task<(SalesShipment? Order, IReadOnlyList<SalesShipmentItem> Items)> GetDetailAsync(Guid id, bool includeItems = true, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    /// <param name="prefix">前缀（销售出库 GI）</param>
