...
/// <summary>
/// 采购单仓储接口（实现见 App.Infrastructure；EF Core + PostgreSQL，销售单 <c>ISalesShipmentRepository</c> 同构）。
/// Repository 只做数据访问，不做业务判定；主表 + 明细的一次写操作由仓储自身持久化。
/// 跨仓储写（单据 + 明细 + 库存 N 行）由 Handler 用 IUnitOfWork 包成同一事务。
/// </summary>
public interface IPurchaseReceiptRepository
{
    ...
    /// <param name="orderId">关联采购订单 id，可空（按订单查入库单）</param>
    ...
    /// <param name="cancellationToken">取消令牌</param>
    Task AddAsync(PurchaseReceipt order, IReadOnlyList<PurchaseReceiptItem> items, CancellationToken cancellationToken = default);
    ...
    /// </summary>
    /// <param name="orderIds">采购单 id 集合（空集合返回空列表）</param>
    /// <param name="cancellationToken">取消令牌</param>
    Task<IReadOnlyList<PurchaseReceiptItem>> GetItemsByOrderIdsAsync(IReadOnlyCollection<Guid> orderIds, CancellationToken cancellationToken = default);
}
