...
namespace App.Core.Features.PurchaseReturns.ExportPurchaseReturns;

/// <summary>
/// 采购退货单导出用例（erp-export）：复用列表筛选取全量单据 + 一次批量取明细分组，
/// 输出「单据 + 明细」两个工作表（明细首列为所属单号）；作废单据照常导出，便于对账核对
/// </summary>
public sealed class ExportPurchaseReturnsRequestHandler : IRequestHandler<ExportPurchaseReturnsRequest, ExportResultDto>
{
    ...
    /// 初始化采购退货单导出用例处理器
    /// </summary>
    public ExportPurchaseReturnsRequestHandler(
    ...
    public async Task<ExportResultDto> HandleAsync(ExportPurchaseReturnsRequest request, CancellationToken cancellationToken = default)
    {
        var (orders, _) = await _purchaseReturnRepository.GetPagedAsync(
            request.Keyword,
            request.PartnerId,
            request.Start,
            request.End,
        ...
        var items = await _purchaseReturnRepository.GetItemsByReturnIdsAsync(orderIds, cancellationToken);
        ...
        var itemsByOrder = items.ToLookup(i => i.ReturnId);
        ...
        {
            FileName = ExportFileNames.Build(ExportDomainNames.PurchaseReturns, DateTimeOffset.UtcNow),
