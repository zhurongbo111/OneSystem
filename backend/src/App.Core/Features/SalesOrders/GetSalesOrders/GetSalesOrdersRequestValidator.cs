...
/// <summary>
/// 销售订单分页查询请求格式校验：只做数据格式检查
/// </summary>
public sealed class GetSalesOrdersRequestValidator : AbstractValidator<GetSalesOrdersRequest>
{
    /// <summary>
    /// 初始化校验规则
    /// </summary>
    public GetSalesOrdersRequestValidator()
    {
        ...
        RuleFor(x => x.FlowStatus)
            .Must(s => s is null
                or OrderFlowStatus.Voided
                or OrderFlowStatus.Pending
                or OrderFlowStatus.Partial
                or OrderFlowStatus.Completed
                or OrderFlowStatus.Closed)
            .WithMessage("订单状态取值非法");
