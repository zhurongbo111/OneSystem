...
/// <summary>
/// 销售订单实体 → 出参映射（实体字段直接映射为 DTO，禁止把实体暴露到 API）；
/// 未执行量（Quantity − FulfilledQuantity）等派生字段在 Mapper 内计算。
/// </summary>
internal static class SalesOrdersDtoMapper
{
    /// <summary>
    /// 主表实体 + 明细行实体转详情 DTO
    /// </summary>
    ...
    /// 主表实体 + 未发数量合计转列表 DTO
    /// </summary>
    public static SalesOrderListItemDto ToSalesOrderListItemDto(SalesOrder order, int unfulfilledQuantity)
        => new()
        {
            Id = order.Id.ToString(),
            OrderNo = order.OrderNo,
