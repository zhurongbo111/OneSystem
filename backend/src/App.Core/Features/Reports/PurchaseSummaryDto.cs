...
/// <summary>
/// 采购汇总行出参（净额 = 入库 − 退货，由 Mapper 计算；specs/025-erp-report/design.md §0.1）。
/// </summary>
public sealed class PurchaseSummaryItemDto
{
    /// <summary>分组键（供应商 id 或商品 id；理论不为空，保留可空以容忍历史脏数据）</summary>
    public string? Key { get; init; }

    /// <summary>分组名称（供应商名称或商品名称）</summary>
    public string Name { get; init; } = string.Empty;

    /// <summary>计量单位（仅商品维度有值）</summary>
    public string? Unit { get; init; }

    /// <summary>入库单数</summary>
    public int OrderCount { get; init; }

    /// <summary>入库数量</summary>
    public int InboundQuantity { get; init; }

    /// <summary>入库金额</summary>
    ...
    /// <summary>净数量 = 入库数量 − 退货数量</summary>
    public int NetQuantity { get; init; }

    /// <summary>净额 = 入库金额 − 退货金额</summary>
    public decimal NetAmount { get; init; }
}

/// <summary>
/// 采购汇总合计出参（全量筛选结果口径；净额由 Mapper 计算）。
/// </summary>
public sealed class PurchaseSummaryTotalDto
{
