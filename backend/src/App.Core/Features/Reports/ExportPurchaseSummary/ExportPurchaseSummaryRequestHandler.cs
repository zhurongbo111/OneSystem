...
namespace App.Core.Features.Reports.ExportPurchaseSummary;

/// <summary>
/// 采购汇总导出用例（erp-export）：复用报表聚合查询取全量，导出列 = 报表列定义（去序号 / 操作列），
/// 末行输出与页面口径一致的合计行（入库单数 / 入库数量与金额 / 退货数量与金额 / 净数量与净额）
/// </summary>
public sealed class ExportPurchaseSummaryRequestHandler : IRequestHandler<ExportPurchaseSummaryRequest, ExportResultDto>
{
    ...
    public ExportPurchaseSummaryRequestHandler(
    ...
    public async Task<ExportResultDto> HandleAsync(
        ExportPurchaseSummaryRequest request, CancellationToken cancellationToken = default)
    {
        // 期间必填由 Validator 保证（全局校验在分发前执行）
        var groupByProduct = request.GroupBy == SummaryGroupBy.Product;
        var (items, _, summary) = await _reportQueryRepository.GetPurchaseSummaryAsync(
            request.Start!.Value,
            request.End!.Value,
            request.PartnerId,
            groupByProduct,
        ...
        // 单位列：商品维度为商品单位；往来单位维度聚合的是多种商品、无单一单位 → 输出占位符（不留空白单元格）
        var rows = items
            .Select(item => ReportsDtoMapper.ToPurchaseSummaryItemDto(item))
        ...
        var total = ReportsDtoMapper.ToPurchaseSummaryTotalDto(summary);

        var sheet = new ExcelSheetModel
        {
            Name = ExportDomainNames.PurchaseSummary,
            Columns =
            [
                new ExcelColumnModel
                {
                    Header = groupByProduct ? "商品" : "供应商",
                    ValueType = ExcelValueType.Text,
                    Width = 20,
                },
                new ExcelColumnModel { Header = "单位", ValueType = ExcelValueType.Text, Width = 8 },
                new ExcelColumnModel { Header = "入库单数", ValueType = ExcelValueType.Integer, Width = 10 },
                new ExcelColumnModel { Header = "入库数量", ValueType = ExcelValueType.Integer, Width = 12 },
                new ExcelColumnModel { Header = "入库金额", ValueType = ExcelValueType.Decimal, Width = 14 },
        ...
        {
            FileName = ExportFileNames.Build(ExportDomainNames.PurchaseSummary, DateTimeOffset.UtcNow),
