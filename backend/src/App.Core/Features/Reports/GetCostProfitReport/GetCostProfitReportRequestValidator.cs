...
/// <summary>
/// 成本与毛利报表查询请求格式校验：期间必填与先后、期间上限、分组维度、分页边界。
/// </summary>
public sealed class GetCostProfitReportRequestValidator : AbstractValidator<GetCostProfitReportRequest>
{
    /// <summary>
    /// 初始化校验规则
    /// </summary>
    public GetCostProfitReportRequestValidator()
    {
        RuleFor(x => x.Start)
