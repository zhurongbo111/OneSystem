...
namespace App.Core.Features.PurchaseReceipts.ExportPurchaseReceipts;

/// <summary>
/// 采购入库单导出用例（erp-export）：复用列表筛选取全量单据 + 一次批量取明细分组，
/// 输出「单据 + 明细」两个工作表（明细首列为所属单号）；作废单据照常导出，便于对账核对
/// </summary>
public sealed class ExportPurchaseReceiptsRequestHandler : IRequestHandler<ExportPurchaseReceiptsRequest, ExportResultDto>
{
    ...
    /// 初始化采购入库单导出用例处理器
    /// </summary>
    public ExportPurchaseReceiptsRequestHandler(
    ...
    public async Task<ExportResultDto> HandleAsync(ExportPurchaseReceiptsRequest request, CancellationToken cancellationToken = default)
    {
        var (orders, _) = await _purchaseReceiptRepository.GetPagedAsync(
            request.Keyword,
            request.PartnerId,
            request.OrderId,
            request.Start,
        ...
        var items = await _purchaseReceiptRepository.GetItemsByOrderIdsAsync(orderIds, cancellationToken);
        ...
        var itemsByOrder = items.ToLookup(i => i.ReceiptId);
        ...
        {
            FileName = ExportFileNames.Build(ExportDomainNames.PurchaseReceipts, DateTimeOffset.UtcNow),
