...
/// <summary>
/// 新增销售出库单请求格式校验：只做数据格式检查。
/// 存在性 / 类型匹配 / 订单状态 / 未发数量等查库约束在 Handler 中判断（后端规则 §4.1）。
/// </summary>
public sealed class CreateSalesShipmentRequestValidator : AbstractValidator<CreateSalesShipmentRequest>
{
    /// <summary>
    /// 初始化校验规则
    /// </summary>
    public CreateSalesShipmentRequestValidator()
    {
        ...
        // 业务层（订单存在 / 状态 / 明细归属 / 未发数量）在 Handler 判断
        RuleFor(x => x)
            .Must(request =>
            {
