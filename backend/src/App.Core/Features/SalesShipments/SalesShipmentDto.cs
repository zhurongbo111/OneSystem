...
/// <summary>
/// 销售出库单出参共享模型（主表 + 明细，与后端 DTO camelCase 一一对应；列表行用 SalesShipmentListItemDto）
/// </summary>
public sealed class SalesShipmentDetailDto
{
    ...
    /// <summary>单号</summary>
    public required string ShipmentNo { get; init; }
    ...
    public required IReadOnlyList<SalesShipmentItemDto> Items { get; init; }
}

/// <summary>销售出库单明细出参模型（快照字段原样返回）</summary>
public sealed class SalesShipmentItemDto
{
    ...
    /// <summary>关联销售订单明细行 id（可空：不关联订单的直通单据）</summary>
    public string? OrderItemId { get; init; }
}

/// <summary>销售出库单列表行出参模型（含 Status 供前端作废行置灰、OrderNo 供跟单）</summary>
public sealed class SalesShipmentListItemDto
{
    /// <summary>出库单 id</summary>
    public required string Id { get; init; }
    ...
    /// <summary>数量合计（= Σ 明细数量；订单详情「关联出库单」跟单展示）</summary>
    public required int TotalQuantity { get; init; }
