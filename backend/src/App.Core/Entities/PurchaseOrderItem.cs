...
/// <summary>
/// 采购订单明细实体（对应 PostgreSQL 表 PurchaseOrderItems，销售订单 <c>SalesOrderItem</c> 同构）。
...
public sealed class PurchaseOrderItem
{
    ...
    /// <summary>订单 ID（外键 → PurchaseOrders(Id)，索引）</summary>
    ...
    /// <summary>累计已收数量（由采购入库单关联回写 / 作废回退；未执行量 = Quantity − FulfilledQuantity 由后端推导，不落列）</summary>
