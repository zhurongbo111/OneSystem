...
public sealed class PurchaseReturnRepository : IPurchaseReturnRepository
{
    ...
    /// <inheritdoc />
    public async Task<(PurchaseReturn? Return, IReadOnlyList<PurchaseReturnItem> Items)> GetDetailAsync(Guid id, bool includeItems = true, CancellationToken cancellationToken = default)
    {
        ...
        if (!includeItems)
        {
            return (purchaseReturn, Array.Empty<PurchaseReturnItem>());
        }
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<PurchaseReturnItem>> GetItemsByReturnIdsAsync(IReadOnlyCollection<Guid> returnIds, CancellationToken cancellationToken = default)
    {
        if (returnIds.Count == 0)
        {
            return Array.Empty<PurchaseReturnItem>();
        }

        // 导出用批量取明细（一次查询避免逐单 N+1）；按明细 Id 升序即插入顺序
        return await _dbContext.PurchaseReturnItems.AsNoTracking()
