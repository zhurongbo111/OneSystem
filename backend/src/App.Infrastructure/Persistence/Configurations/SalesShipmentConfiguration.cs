...
/// <summary>
/// 销售单表映射配置（表名 SalesShipments，单号唯一索引；外键不级联删除，业务对象只停用 / 作废不删除）
/// </summary>
internal sealed class SalesShipmentConfiguration : IEntityTypeConfiguration<SalesShipment>
{
    /// <inheritdoc />
    public void Configure(EntityTypeBuilder<SalesShipment> builder)
    {
        builder.ToTable("SalesShipments");
        builder.HasKey(p => p.Id);

        // 长度取自 OrderFieldConstraints（单一来源），与各 RequestValidator 保持一致
        builder.Property(p => p.ShipmentNo)
        ...
        builder.HasIndex(p => p.ShipmentNo).IsUnique();
