...
internal sealed class FakePurchaseOrderRepository : IPurchaseOrderRepository
{
    ...
    public IReadOnlyCollection<PurchaseOrder> Orders => _orders.Values;
    ...
    public IReadOnlyList<PurchaseOrderItem> ItemsOf(Guid orderId)
        => _items.TryGetValue(orderId, out var list) ? list : Array.Empty<PurchaseOrderItem>();

    public Task<(IReadOnlyList<(PurchaseOrder Order, int UnfulfilledQuantity)> Items, int Total)> GetPagedAsync(
        ...
        int page, int pageSize, CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<(PurchaseOrder Order, int UnfulfilledQuantity)> empty = Array.Empty<(PurchaseOrder, int)>();
    ...
    public Task<IReadOnlyList<PurchaseOrder>> GetPicksAsync(Guid partnerId, CancellationToken cancellationToken = default)
    {
        IReadOnlyList<PurchaseOrder> picks = _orders.Values
    ...
    public Task UpdateAsync(PurchaseOrder order, IReadOnlyList<PurchaseOrderItem> items, CancellationToken cancellationToken = default)
    {
        _calls?.Add("Update");
