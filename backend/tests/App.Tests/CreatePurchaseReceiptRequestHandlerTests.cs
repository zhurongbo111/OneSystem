...
/// <summary>
/// CreatePurchaseReceiptRequestHandler 测试（design.md §6）：
...
/// </summary>
public class CreatePurchaseReceiptRequestHandlerTests
{
    private static readonly DateTimeOffset OrderDate = new(2026, 1, 1, 0, 0, 0, TimeSpan.Zero);

    private static (AppDbContext Context, StubCurrentUser User, FakePurchaseReceiptRepository Orders,
        FakeInventoryRepository Inventory, FakeStockMovementRepository Movements, RecordingUnitOfWork Uow,
        CreatePurchaseReceiptRequestHandler Handler, List<string> Calls) CreateHandler()
    {
    ...
    private static CreatePurchaseReceiptRequest RequestWith(Guid partnerId, params (Guid ProductId, int Qty, decimal Price)[] lines)
        => new()
        {
            PartnerId = partnerId,
            OrderDate = OrderDate,
            Items = lines.Select(l => new CreatePurchaseReceiptItem { ProductId = l.ProductId, Quantity = l.Qty, UnitPrice = l.Price }).ToList(),
        };
    ...
    public async Task 新增采购单_成功_应生成单号并重算小计总额且逐行库存增加()
    {
        ...
        // 单号：GR + yyyyMMdd + 4 位序号
        Assert.Matches("^GR20260101\\d{4}$", result.ReceiptNo);
        ...
        Assert.All(movements.Appended, m =>
        {
            ...
            Assert.Equal(result.ReceiptNo, m.SourceNo);
        });
    ...
    public async Task 新增采购单_当天已有单据_单号序号应递增()
    {
        ...
        Assert.Matches("^GR20260101\\d{4}$", first.ReceiptNo);
        Assert.Matches("^GR20260101\\d{4}$", second.ReceiptNo);
        Assert.NotEqual(first.ReceiptNo, second.ReceiptNo);
        Assert.True(
            int.Parse(second.ReceiptNo[^4..]) > int.Parse(first.ReceiptNo[^4..]),
            $"单号序号应递增：{first.ReceiptNo} → {second.ReceiptNo}");
    }
    ...
    public async Task 新增采购单_明细为空_应报OrderItemsEmpty()
    {
        ...
        var ex = await Assert.ThrowsAsync<BusinessException>(() => handler.HandleAsync(new CreatePurchaseReceiptRequest
        {
            ...
            OrderDate = OrderDate,
            Items = Array.Empty<CreatePurchaseReceiptItem>(),
        }));
