...
public class PurchaseReceiptLifecycleTests
{
    ...
    public async Task 作废采购单_成功_应逐行回冲库存并置作废()
    {
        ...
        var handler = new VoidPurchaseReceiptRequestHandler(orders, new FakePurchaseOrderRepository(calls), inventory, movements, GeneralLedgerStubs.NewVouchers(), GeneralLedgerStubs.NewPeriods(), uow, user, TestSupport.AuditLogger);
    ...
    public async Task 作废采购单_不存在_应报NotFound()
    {
        ...
        var handler = new VoidPurchaseReceiptRequestHandler(orders, new FakePurchaseOrderRepository(), inventory, new FakeStockMovementRepository(), GeneralLedgerStubs.NewVouchers(), GeneralLedgerStubs.NewPeriods(), uow, user, TestSupport.AuditLogger);
    ...
    [Fact]
    public async Task 作废采购单_已核销_应报OrderSettledCannotVoid且不回冲()
    {
        var (orders, inventory, uow, user, order, p1, _, calls) = SeedNormal();
        ...
        Assert.Contains(order.ReceiptNo, ex.Message);
