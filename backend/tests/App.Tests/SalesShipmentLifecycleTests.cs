...
public class SalesShipmentLifecycleTests
{
    ...
    public async Task 作废销售单_成功_应逐行回冲库存并置作废()
    {
        ...
        var handler = new VoidSalesShipmentRequestHandler(orders, new FakeSalesOrderRepository(calls), inventory, movements, GeneralLedgerStubs.NewVouchers(), GeneralLedgerStubs.NewPeriods(), uow, user, TestSupport.AuditLogger);
    ...
    public async Task 作废销售单_不存在_应报NotFound()
    {
        ...
        var handler = new VoidSalesShipmentRequestHandler(orders, new FakeSalesOrderRepository(), inventory, new FakeStockMovementRepository(), GeneralLedgerStubs.NewVouchers(), GeneralLedgerStubs.NewPeriods(), uow, user, TestSupport.AuditLogger);
