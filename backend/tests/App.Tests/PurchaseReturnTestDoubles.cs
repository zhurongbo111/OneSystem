...
{
    private readonly Dictionary<Guid, PurchaseReturn> _returns = [];
    private readonly Dictionary<Guid, List<PurchaseReturnItem>> _items = [];
    ...
    public Task<(PurchaseReturn? Return, IReadOnlyList<PurchaseReturnItem> Items)> GetDetailAsync(Guid id, bool includeItems = true, CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<PurchaseReturnItem> items = includeItems
            ? _items[id].OrderBy(i => i.Id).ToList() // 按 Id 还原插入顺序
            : Array.Empty<PurchaseReturnItem>();
    ...
    public Task<IReadOnlyList<PurchaseReturnItem>> GetItemsByReturnIdsAsync(IReadOnlyCollection<Guid> returnIds, CancellationToken cancellationToken = default)
    {
        _calls?.Add("GetItemsByReturnIds");
        IReadOnlyList<PurchaseReturnItem> items = _items
    ...
    public Task AddSettledAmountAsync(Guid id, decimal delta, Guid? operatorId, CancellationToken cancellationToken = default)
    {
        ...
        purchaseReturn.SettledAmount += delta;
